...
function openHkgCreateMeetingModal(){
  ...
  $('modal').innerHTML=`<div class="modal-card"><div class="modal-h"><h3>＋ Tạo lịch họp không giấy</h3><button class="modal-close" onclick="closeModal()">✕</button></div><div class="modal-b">
    ...
    <div class="form-grid" style="margin-top:14px">
      ...
      <div class="field full"><label class="hkg-checkline"><input id="hkgNewLinkCalendar" type="checkbox"> Liên thông với Lịch công tác</label><div class="hint">Chỉ khi tích mục này, cuộc họp mới được tạo thêm trong Lịch công tác. Nếu không tích, cuộc họp chỉ nằm trong module Họp không giấy.</div></div>
    </div>
    <div class="toolbar" style="margin-top:14px"><button class="btn primary" onclick="saveHkgNewMeeting()">Tạo cuộc họp và mở hồ sơ</button><button class="btn ghost" onclick="closeModal()">Đóng</button></div>
  </div></div>`;
}
...
async function saveHkgNewMeeting(){
  ...
  const form={
    ...
    linkCalendar: !!($('hkgNewLinkCalendar') && $('hkgNewLinkCalendar').checked),
    loaiLich: 'HOP',
    trangThai: 'PUBLISHED'
  };
  try{
 ...
 * ============================================================ */
const HKG_V99_VERSION = 'hkg_v99_optional_calendar_link';
...
async function renderHkgList(){
  ...
  try{
    ...
    box.innerHTML=`<div class="hkg-shell hkg-v98">
      <div class="hkg-hero">
        <div><h2>📋 Họp không giấy</h2><p>Hồ sơ họp điện tử. Khi tạo cuộc họp có thể chọn liên thông sang Lịch công tác hoặc chỉ lưu nội bộ trong Họp không giấy.</p></div>
...
function hkgMeetingListHtml(arr){
  if(!arr||!arr.length)return '<div class="empty"><b>Chưa có hồ sơ Họp không giấy.</b><br>Chỉ những lịch đã tạo/liên kết hồ sơ mới hiển thị tại đây. Có thể tạo trực tiếp bằng nút <b>＋ Tạo cuộc họp</b>. Chỉ cuộc họp tích <b>Liên thông với Lịch công tác</b> mới sinh thêm lịch bên Lịch công tác; hoặc vào Lịch công tác bấm <b>📋 Hồ sơ họp</b> để liên kết lịch có sẵn.</div>';
  ...
  try{
    ...
    box.innerHTML=`<div class="hkg-shell hkg-v98">
      <div class="hkg-detail-head v98">
        <div class="hkg-detail-main"><div><button class="hkg-back" onclick="APP.hkg.view='list';renderHkgList()">← Danh sách</button><div class="hkg-detail-title">${esc(m.noiDung||'Hồ sơ họp')}</div><div class="hkg-detail-meta"><span><b>${esc(m.meetingId)}</b></span><span>Ngày: ${esc(dateVNShort(m.ngayHienThi||m.ngayBatDau))}</span><span>Giờ: ${esc(timeRange(m))}</span><span>Khối: ${esc(hkgMeetingBlock(m))}</span><span class="hkg-status ${hkgStatusClass(dossier)}">${esc(hkgStatusLabel(dossier))}</span></div></div><div class="hkg-toolbar">${m.linkCalendar?`<button class="btn sm" onclick="openDetail('${esc(id)}')">👁 Lịch gốc</button>`:`<span class="hkg-link-chip internal">Chỉ trong Họp không giấy</span>`}<button class="btn sm" onclick="hkgReloadDetail(APP.hkg.tab)">↻ Tải lại</button></div></div>
